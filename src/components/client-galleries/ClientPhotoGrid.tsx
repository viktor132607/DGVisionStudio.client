                ...
                {photos.map((photo, index) => {
                    ...
                    return (
                        <div key={photo.id} className="group relative aspect-[4/5] w-full overflow-hidden bg-neutral-100 dark:bg-zinc-900">
                            ...
                            <div className="pointer-events-none absolute inset-0 bg-black/0 transition duration-300 group-hover:bg-black/20 dark:group-hover:bg-black/30" />
                            <div className="pointer-events-none absolute inset-x-0 bottom-0 translate-y-3 p-3 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100 sm:translate-y-4 sm:p-4">
                                <p className="text-[10px] uppercase tracking-[0.22em] text-white/70 sm:tracking-[0.28em]">#{photo.displayOrder}</p>
                                <p className="mt-1 text-sm font-semibold text-white sm:text-[15px]">{title}</p>
                            </div>

                            {photo.canDownload ? (
                                <a href={getPhotoDownloadUrl(photo.id)} className="absolute bottom-3 right-3 z-10 inline-flex items-center justify-center rounded-full bg-black/65 px-3 py-1.5 text-[11px] font-bold text-white backdrop-blur-sm transition hover:bg-black/85">
