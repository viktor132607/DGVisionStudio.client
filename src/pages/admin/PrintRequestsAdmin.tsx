...
export default function PrintRequestsAdmin() {
    ...
    const getPreviewImageUrl = (item: PrintRequestItemDto) => {
        const rawUrl = item.thumbnailUrl || item.imageUrl || ""
        return rawUrl ? resolveAssetUrl(rawUrl) : ""
    }

    const getPhotoId = (item: PrintRequestItemDto) => item.portfolioImageId || item.id
    ...
    const downloadSelected = () => {
        ...
        selectedItems.forEach((item, index) => window.setTimeout(() => downloadItem(previewRequest, item), index * 300))
    }
    ...
    const statuses = useMemo(() => Array.from(new Set(requests.map(x => normalizeStatus(x.status)).filter(Boolean))).sort(), [requests])
        ...
        <div className="p-6">
            ...
            {!loading ? (
                <>
                    <div className="overflow-x-auto">
                        <table className="w-full overflow-hidden rounded-xl border bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                            <thead className="bg-gray-50 dark:bg-zinc-950">
                                <tr>
                                    <th className="p-3 text-left text-gray-700 dark:text-zinc-300">ID</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Албум</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Клиент</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Email</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Телефон</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Снимки</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Статус</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Бележки</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Дата</th><th className="p-3 text-left text-gray-700 dark:text-zinc-300">Действия</th>
                                </tr>
                            </thead>
                            <tbody>
                                {pagedRequests.length > 0 ? pagedRequests.map(request => {
                                    ...
                                    return (
                                        <tr key={request.id} className="border-b transition last:border-b-0 hover:bg-gray-50 dark:border-zinc-800 dark:hover:bg-zinc-950">
                                            ...
                                            <td className="p-3"><div className="flex flex-col"><span className="font-medium text-gray-900 dark:text-white">{request.albumTitle || "-"}</span>{isClientGallery ? <span className="mt-1 w-fit rounded-full bg-blue-100 px-2 py-0.5 text-xs font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">Client print upload</span> : null}{!request.isSeenByAdmin ? <span className="mt-1 w-fit rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700 dark:bg-red-500/10 dark:text-red-300">Нова</span> : null}</div></td>
                                            ...
                                            <td className="p-3"><select value={status} disabled={isBusy} onChange={event => void changeStatus(request.id, event.target.value)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-white"><option value="New">New</option><option value="InProgress">InProgress</option><option value="Completed">Completed</option><option value="Cancelled">Cancelled</option></select></td>
                                            ...
                                            <td className="p-3"><div className="flex flex-nowrap items-center gap-2 whitespace-nowrap"><button type="button" onClick={() => openPreview(request)} className="rounded-lg bg-slate-700 px-3 py-1.5 text-white hover:bg-slate-800">Преглед</button><button type="button" onClick={() => downloadAll(request)} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-white hover:bg-emerald-700">Изтегли всички</button><button type="button" onClick={() => printRequest(request)} className="rounded-lg bg-blue-600 px-3 py-1.5 text-white hover:bg-blue-700">Print</button>{!request.isSeenByAdmin ? <button type="button" disabled={isBusy} onClick={() => void markSeen(request.id)} className="rounded-lg bg-green-600 px-3 py-1.5 text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-60">Видяна</button> : null}<button type="button" onClick={() => void remove(request.id)} disabled={isBusy} className="rounded-lg bg-red-600 px-3 py-1.5 text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60">{isBusy ? "..." : "Delete"}</button></div></td>
                                        </tr>
                                    )
                    ...
                    <div className="mt-6 flex flex-wrap items-center justify-center gap-2"><button disabled={safeCurrentPage === 1} onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800">Prev</button>{pageNumbers.map(page => <button key={page} onClick={() => setCurrentPage(page)} className={`rounded-lg border px-3 py-1.5 ${page === safeCurrentPage ? "border-gray-900 bg-gray-900 text-white dark:border-white dark:bg-white dark:text-black" : "border-gray-300 bg-white hover:bg-gray-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800"}`}>{page}</button>)}<button disabled={safeCurrentPage === totalPages} onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800">Next</button></div>
                </>
            ) : null}
            {previewRequest ? (
